...
public abstract class TemplateManager implements ITemplateManager {
	 ...
	 * @since HOZDoEAS7.0 1.0
	 */
	final java.util.Map prepareData(java.awt.Component parentUI, String currentDirectory, boolean enableMultiple) throws FileNotFoundException, IOException {
		java.util.Map excelsDataMap = null;
		if (enableBeforeReadFile()) {
			excuteBeforeReadFile();
		}
		// Excel
		java.io.File[] files = importExcel(parentUI, currentDirectory, enableMultiple);
	 ...
	 * @since HOZDoEAS7.0 1.0
	 */
	final java.io.File[] importExcel(java.awt.Component parentUI, String currentDirectory, boolean enableMultiple) throws FileNotFoundException {
		java.io.File[] files = null;
		if (enableBeforeReadFile()) {
			excuteBeforeReadFile();
		}
		// Excel
		files = this.excelUtils.importExcel(parentUI, currentDirectory, enableMultiple);
		return files;
	}
